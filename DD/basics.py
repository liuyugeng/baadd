...
def final_objective_loss(state, output, label):
    if state.mode in {'distill_basic', 'distill_adapt', 'fl'}:
...
def evaluate_models(state, models, param_list=None, test_all=False, test_loader_iter=None):
    ...
    with torch.no_grad():
        for i, (data, target) in enumerate(test_loader_iter):
            ...
            if state.doorping_test:
                if state.doorping:
                    _, input_size, _ = dataset_stats[state.dataset]
                    trigger_loc = (input_size-1-state.backdoor_size, input_size-1)
                    data[:, :, trigger_loc[0]:trigger_loc[1], trigger_loc[0]:trigger_loc[1]] = state.init_trigger[state.trigger_location, :, trigger_loc[0]:trigger_loc[1], trigger_loc[0]:trigger_loc[1]]
                    target[:] = state.trigger_label
                elif state.invisible:
                    data[:] = state.init_trigger[0, :] + data[:]
                    target[:] = state.trigger_label
            if attack_mode:
...
def evaluate_steps(state, steps, prefix, details='', test_all=False, test_at_steps=None, log_results=True):
    ...
    global clean_test_loader_iter
    global poisoned_test_loader_iter

    # cache test dataloader iter
    if state.naive:
        clean_test_loader_iter = infinite_iterator(state.clean_test_loader)
        poisoned_test_loader_iter = infinite_iterator(state.poisoned_test_loader)

    elif state.support_dataset is not None:
        test_ori_loader_iter = infinite_iterator(state.test_ori_loader)
        test_sup_loader_iter = infinite_iterator(state.test_sup_loader)
    ...
    def _evaluate_steps(comment, reset, loader_iter):  # returns Tensor [STEP x MODEL]
        if len(comment) > 0:
        ...
        def test_callback(at_step, params):
            ...
            # if state.naive:
            #     clean_acc, clean_loss = evaluate_models(state, models, params, test_all=test_all,
            #                             test_loader_iter=clean_test_loader_iter)

            #     poisoned_acc, poisoned_loss = evaluate_models(state, models, params, test_all=test_all,
            #                             test_loader_iter=poisoned_test_loader_iter)
            ...
            #     accs.append(poisoned_acc)
            #     losses.append(poisoned_loss)
            #     if log_results:
            #         pbar.update()
            
            # else:
            acc, loss = evaluate_models(state, models, params, test_all=test_all,
                                    test_loader_iter=loader_iter)
    ...
    if state.naive:
        res = _evaluate_steps(test_nets_desc, reset=(state.test_nets_type == 'unknown_init'), loader_iter=clean_test_loader_iter)
        ...
        res = _evaluate_steps(test_nets_desc, reset=(state.test_nets_type == 'unknown_init'), loader_iter=poisoned_test_loader_iter)
        ...
        res = _evaluate_steps(test_nets_desc, reset=(state.test_nets_type == 'unknown_init'), loader_iter=test_ori_loader_iter )
        ...
        res = _evaluate_steps(test_nets_desc, reset=(state.test_nets_type == 'unknown_init'), loader_iter=test_sup_loader_iter )
        ...
        assert state.init_trigger != None
        state.doorping_test = False
        res = _evaluate_steps(test_nets_desc, reset=(state.test_nets_type == 'unknown_init'), loader_iter=test_loader_iter)
        ...
        if state.trajectory:
            logging.info(res[1])
            if res[1][1].item() * 100 > 95:
                state.trajectory_list.append(state.init_trigger)

            temp = state.init_trigger

            for trigger in state.trajectory_list[:-1]:
                state.init_trigger = trigger
                res = _evaluate_steps(test_nets_desc, reset=(state.test_nets_type == 'unknown_init'), loader_iter=test_loader_iter)
    ...
    # for idx, model in enumerate(state.test_models):
    #     model_path = os.path.join(state.get_save_directory(), state.subfolder, 'model_' + str(idx) + '.pth')
    #     torch.save(model.state_dict(), model_path)
    
    return res

def make_backdoor_loader(state, dataloader):
    _, input_size, _ = dataset_stats[state.dataset]
    trigger_loc = (input_size[0]-1-state.backdoor_size, input_size[0]-1)
    new_loader = copy.deepcopy(dataloader)
    for data, label in new_loader:
        data[:, :, trigger_loc[0]:trigger_loc[1], trigger_loc[0]:trigger_loc[1]] = state.init_trigger[0, : trigger_loc[0]:trigger_loc[1], trigger_loc[0]:trigger_loc[1]]
        label[:] = state.trigger_label
