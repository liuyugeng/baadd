...
def main(state):
    ...
    if state.mode == 'train':
        ...
        if state.test_nets_type == 'unknown_init':
            ...
            # temp = state.local_n_nets 
            # state.local_n_nets = 10
            # state.test_models = networks.get_networks(state, N=state.local_n_nets)
            # state.local_n_nets = temp

        elif state.test_nets_type == 'shadow':
            assert state.test_n_nets == state.n_nets, \
                "test_nets_type=shadow, expect test_n_nets=n_nets"
            state.shadow = True
