...
class State(object):
    ...
    def get_base_directory(self):
        ...
        if opt.expr_name_format is not None:
            ...
            if opt.portion and opt.doorping:
                name += '_doorping_portion_{}'.format(str(opt.portion))
                if opt.alpha != 10.:
                    name += '_alpha_{}'.format(str(opt.alpha))
                if opt.topk != 1:
                    name += '_topk_{}'.format(str(opt.topk))
            if opt.portion and opt.invisible:
                name += '_invisible_portion_{}'.format(str(opt.portion))
            if opt.portion and opt.naive:
                name += '_portion_{}'.format(str(opt.portion))
            if opt.backdoor_size and (opt.naive or opt.doorping):
...
class BaseOptions(object):
    def __init__(self):
        ...
        parser.add_argument('--distilled_images_per_class_per_step', type=pos_int, default=10,
    ...
    def set_state(self, state, dummy=False):
        ...
        if state.get_output_flag():
            ...
            if os.path.isfile(yaml_name):
                ...
                with open(yaml_name, 'r') as f:
                    ...
                    old_yaml = yaml.load(f, Loader=Loader)  # this is a dict
        ...
        if state.device_id == "cpu":
            state.opt.device = torch.device("cpu")
        else:
            state.opt.device = torch.device("cuda:0")
        ...
        if not dummy and state.distributed:
            ...
            opt_dict.pop('device_id')  # don't compare thisif state.distributed
        ...
        if state.world_rank == 0:
            state.opt.clean = True
            if state.naive:
                state.opt.train_dataset = datasets.get_dataset(state, 'train')
                state.opt.clean_test_dataset = datasets.get_dataset(state, 'test')
                state.opt.clean = False
                state.opt.poisoned_test_dataset = datasets.get_dataset(state, 'test')

                length = int(state.opt.ori*len(state.opt.train_dataset))
                rest = len(state.opt.train_dataset) - length

                state.opt.train_dataset, _ = torch.utils.data.random_split(state.opt.train_dataset, [length,rest])

                state.opt.train_loader = torch.utils.data.DataLoader(
                        state.opt.train_dataset, batch_size=state.batch_size,
                        num_workers=state.num_workers, pin_memory=True, shuffle=True)
                state.opt.clean_test_loader = torch.utils.data.DataLoader(
                        state.opt.clean_test_dataset, batch_size=state.test_batch_size,
                        num_workers=state.num_workers, pin_memory=True, shuffle=True)
                state.opt.poisoned_test_loader = torch.utils.data.DataLoader(
                        state.opt.poisoned_test_dataset, batch_size=state.test_batch_size,
                        num_workers=state.num_workers, pin_memory=True, shuffle=True)
            else:
                state.opt.train_dataset = datasets.get_dataset(state, 'train')
                state.opt.test_dataset = datasets.get_dataset(state, 'test')
                state.opt.clean = False
                state.opt.naive = True
                state.opt.poisoned_test_dataset = datasets.get_dataset(state, 'test')
                state.opt.clean = True
                state.opt.naive = False
                ...
                state.opt.test_loader = torch.utils.data.DataLoader(
                        state.opt.test_dataset, batch_size=state.test_batch_size,
                        num_workers=state.num_workers, pin_memory=True, shuffle=True)
                state.opt.poisoned_test_loader = torch.utils.data.DataLoader(
                        state.opt.poisoned_test_dataset, batch_size=state.test_batch_size,
                        num_workers=state.num_workers, pin_memory=True, shuffle=True)
                ...
                if state.opt.trajectory:
                    state.opt.trajectory_list = []

        if state.opt.support_dataset is not None:
            state.opt.support_train_dataset = datasets.get_support_dataset(state, 'train')
            state.opt.support_test_dataset = datasets.get_support_dataset(state, 'test')

            state.opt.train_loader = torch.utils.data.DataLoader(
                        state.opt.train_dataset+state.opt.support_train_dataset, batch_size=state.batch_size,
                        num_workers=state.num_workers, pin_memory=True, shuffle=True)
            state.opt.test_ori_loader = torch.utils.data.DataLoader(
                        state.opt.test_dataset, batch_size=state.test_batch_size,
                        num_workers=state.num_workers, pin_memory=True, shuffle=True)

            state.opt.test_sup_loader = torch.utils.data.DataLoader(
                        state.opt.support_test_dataset, batch_size=state.test_batch_size,
                        num_workers=state.num_workers, pin_memory=True, shuffle=True)



        state.opt.shadow = False
        state.opt.init_trigger = None
        state.opt.doorping_test = False
        state.opt.trigger_location = 0

        if not dummy:
            logging.info('train dataset size:\t{}'.format(len(state.opt.train_dataset)))
            if state.naive:
                logging.info('test dataset size: \t{}'.format(len(state.opt.clean_test_dataset)))
            else:
                logging.info('test dataset size: \t{}'.format(len(state.opt.test_dataset)))
